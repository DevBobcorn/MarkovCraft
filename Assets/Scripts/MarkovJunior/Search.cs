...
namespace MarkovJunior
{
    static class Search
    {
        private record ItemInfo : IComparable<ItemInfo>
        {
            public double priority;
            public int value;

            public ItemInfo(int v, double p)
            {
                value = v;
                priority = p;
            }

            public int CompareTo(ItemInfo info) // TODO: Check if correct
            {
        ...
        public static byte[][] Run(byte[] present, int[] future, Rule[] rules, int MX, int MY, int MZ, int C, bool all, int limit, double depthCoefficient, int seed)
        {
            ...
            //present.Print(MX, MY);
            int[][] bpotentials = ArrayHelper.Array2D(C, present.Length, -1);
            int[][] fpotentials = ArrayHelper.Array2D(C, present.Length, -1);
            ...
            {
                Debug.Log("INCORRECT PROBLEM");
                return null;
            }
            Debug.Log($"root estimate = ({rootBackwardEstimate}, {rootForwardEstimate})");
            ...
            PriorityQueue<ItemInfo> frontier = new();
            System.Random random = new(seed);
            //frontier.Enqueue(0, rootBoard.Rank(random, depthCoefficient));
            frontier.Enqueue(new(0, rootBoard.Rank(random, depthCoefficient)));
            ...
            while (frontierLength > 0 && (limit < 0 || database.Count < limit))
            {
                ...
                int parentIndex = frontier.Dequeue().value;
                frontierLength--;
                Board parentBoard = database[parentIndex];
                //Debug.Log("-----------------------------------------------------------------------------------");
                //Debug.Log($"extracting board at depth {parentBoard.depth} and estimate ({parentBoard.backwardEstimate}, {parentBoard.forwardEstimate}):");
                ...
                //Debug.Log($"this board has {children.Length} children");
                ...
                //for (int c = 0; c < children.Length; c++)
                {
                    ...
                    if (success)
                    {
                        ...
                        {
                            //Debug.Log($"found a shorter {parentBoard.depth + 1}-route to an existing {oldBoard.depth}-board of estimate ({oldBoard.backwardEstimate}, {oldBoard.forwardEstimate})");
                            ...
                            if (oldBoard.backwardEstimate >= 0 && oldBoard.forwardEstimate >= 0)
                            {
                                ...
                                frontier.Enqueue(new(childIndex, oldBoard.Rank(random, depthCoefficient)));
                                frontierLength++;
                            }
                        }
                        //else Debug.Log($"found a longer {parentBoard.depth + 1}-route to an existing {oldBoard.depth}-board of estimate ({oldBoard.backwardEstimate}, {oldBoard.forwardEstimate})");
                    }
                    else
                    {
                        ...
                        //Debug.Log($"child {c} has estimate ({childBackwardEstimate}, {childForwardEstimate}):");
                        ...
                        {
                            Debug.Log($"found a trajectory of length {parentBoard.depth + 1}, visited {visited.Count} states");
                        ...
                        else
                        {
                            if (limit < 0 && childBackwardEstimate + childForwardEstimate <= record)
                            {
                                ...
                                Debug.Log($"found a state of record estimate {record} = {childBackwardEstimate} + {childForwardEstimate}");
                                childState.Print(MX, MY);
                            }
                            //frontier.Enqueue(childIndex, childBoard.Rank(random, depthCoefficient));
                            frontier.Enqueue(new(childIndex, childBoard.Rank(random, depthCoefficient)));
                            frontierLength++;
                        }
                    }
                }
        ...
        static void Print(this byte[] state, int MX, int MY)
        {
            ...
            {
                for (int x = 0; x < MX; x++)
                    Debug.Log($"{characters[state[x + y * MX]]} ");
        ...
        public static List<byte[]> AllChildStates(this byte[] state, int MX, int MY, Rule[] rules)
        {
            ...
            bool[] mask = ArrayHelper.Array1D(tiles.Length, true);
    ...
    class Board
    {
        ...
        public double Rank(System.Random random, double depthCoefficient)
        {
