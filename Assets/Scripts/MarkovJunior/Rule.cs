...
namespace MarkovJunior
{
    class Rule
    {
        ...
        public Rule(int[] input, int IMX, int IMY, int IMZ, byte[] output, int OMX, int OMY, int OMZ, int C, double p)
        {
            ...
            {
                int w = input[i];
                binput[i] = w == wildcard ? (byte)0xff : (byte)GetTrailingZeroCount(w);
            }
        }
        ...
        public static Rule Load(XElement xelem, Grid gin, Grid gout)
        {
            ...
            else
            {
                ...
                inRect = ArrayHelper.FlatArray3D(FX / 2, FY, FZ, (x, y, z) => rect[x + y * FX + z * FX * FY]);
                outRect = ArrayHelper.FlatArray3D(FX / 2, FY, FZ, (x, y, z) => rect[x + FX / 2 + y * FX + z * FX * FY]);
            }
