...
namespace MarkovJunior
{
    static class Graphics
    {
        ...
        public static Texture2D CreateTexture2D(int[] pixels, int sizeX, int sizeY)
        {
            var texture = new Texture2D(sizeX, sizeY);
            var pixelData = new Color32[sizeX * sizeY];
            for (int x = 0;x < sizeX;x++) for (int y = 0;y < sizeY;y++)
            {
                // Flip vertically
                var i1 = x + y * sizeX;
                var i2 = x + (sizeY - 1 - y) * sizeX;
                pixelData[i2] = ColorConvert.GetColor32(pixels[i1]);
            }
            texture.SetPixels32(pixelData);
            // Update mipmap to ensure it renders correctly at any scale
            texture.Apply(true);
        ...
        public static (int[], int, int) IsometricRender(byte[] state, int MX, int MY, int MZ, int[] colors, int blocksize, int MARGIN)
        {
            ...
            for (int i = 0; i < screen.Length; i++) screen[i] = 0; // Fully transparent, black
