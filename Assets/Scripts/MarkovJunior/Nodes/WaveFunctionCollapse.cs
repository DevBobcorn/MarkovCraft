...
namespace MarkovJunior
{
    abstract class WFCNode : Branch
    {
        ...
        public override bool Go()
        {
            ...
            if (firstgo)
            {
                ...
                if (!firstSuccess)
                {
                    Debug.Log("initial conditions are contradictive");
                    return false;
                }
                ...
                random = new System.Random((int)goodseed);
                stacksize = 0;
            ...
            for (int k = 0; k < tries; k++)
            {
                ...
                while (true)
                {
                    ...
                    if (node >= 0)
                    {
                        ...
                        if (!success)
                        {
                            Debug.Log($"CONTRADICTION on try {k} with {observationsSoFar} observations");
                            break;
                        }
                    }
                    else
                    {
                        Debug.Log($"wfc found a good seed {seed} on try {k} with {observationsSoFar} observations");
                        return seed;
                    }
                }
            }

            Debug.Log($"wfc failed to find a good seed in {tries} tries");
            return null;
        }
    ...
    class Wave
    {
        ...
        {
            data = ArrayHelper.Array2D(length, P, true);
            compatible = ArrayHelper.Array3D(length, P, D, -1);
