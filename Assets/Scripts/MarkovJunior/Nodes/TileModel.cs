...
namespace MarkovJunior
{
    class TileNode : WFCNode
    {
        ...
        override protected bool Load(XElement xelem, bool[] parentSymmetry, Grid grid)
        {
            ...
            byte[] newtile(Func<int, int, int, byte> f) => ArrayHelper.FlatArray3D(S, S, SZ, f);
            ...
            foreach (XElement xtile in xtiles)
            {
                ...
                List<byte[]> localdata = fullSymmetry ? SymmetryHelper.CubeSymmetries(flatTile, zRotate, yRotate, xReflect, ArrayHelper.Same).ToList()
                    : SymmetryHelper.SquareSymmetries(flatTile, zRotate, xReflect, ArrayHelper.Same).ToList();
            ...
            if (!map.ContainsKey(0)) map.Add(0, ArrayHelper.Array1D(P, true));

            bool[][][] tempPropagator = ArrayHelper.Array3D(6, P, P, false);

            int index(byte[] p)
            {
                for (int i = 0; i < tiledata.Count; i++) if (ArrayHelper.Same(p, tiledata[i])) return i;
                return -1;
            };
            ...
            for (int z = 0; z < grid.MZ; z++) for (int y = 0; y < grid.MY; y++) for (int x = 0; x < grid.MX; x++)
                    {
                        ...
                        int[][] votes = ArrayHelper.Array2D(S * S * SZ, newgrid.C, 0);
