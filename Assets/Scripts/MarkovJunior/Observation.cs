...
namespace MarkovJunior
{
    class Observation
    {
        ...
        public static bool ComputeFutureSetPresent(int[] future, byte[] state, Observation[] observations)
        {
            ...
            {
                //Debug.Log($"observed value {k} not present on the grid, observe-node returning false");
                return false;
            }
        ...
        static void ComputePotentials(int[][] potentials, int MX, int MY, int MZ, Rule[] rules, bool backwards)
        {
            ...
            bool[][] matchMask = ArrayHelper.Array2D(rules.Length, potentials[0].Length, false);
